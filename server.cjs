...
app.post("/api/brochure-lead", async (req, res) => {
  const { name, email, phone, message, source, product } = req.body;
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1.0" />
  <title>Brochure Lead — SunTech</title>
</head>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f4f5f7;padding:32px 16px;">
    <tr><td align="center">
      ...
      <table width="600" cellpadding="0" cellspacing="0" border="0"
        ...
        <!-- HEADER -->
        <tr>
          <td align="center" style="background:#ffffff;padding:36px 32px 24px;border-bottom:1px solid #f0f0f0;">
            ${logoSrc
              ? `<img src="${logoSrc}" alt="SunTech Packaging Machines" width="150" style="display:block;height:auto;object-fit:contain;border:0;margin:0 auto 20px;"/>`
              : `<h1 style="margin:0 0 20px;font-size:22px;color:#111827;">SunTech Packaging</h1>`
            }

            <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto 16px;">
              <tr>
                <td style="width:44px;height:44px;background:#fef3e2;border-radius:50%;text-align:center;vertical-align:middle;font-size:20px;">
                  🔔
                </td>
              </tr>
            </table>
            ...
            <h2 style="margin:0 0 6px;font-size:20px;font-weight:700;color:#111827;">
              New Brochure Lead Received
            </h2>
            <p style="margin:0;font-size:13px;color:#6b7280;">
              A new brochure download enquiry was submitted from your website.
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:28px 32px 8px;">
            <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:0.14em;
                       text-transform:uppercase;color:#e8961a;">
              Lead Details
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              ${[
                ...
                ["Email", email],
                ["Phone", phone || "N/A"],
                ["Product", product || "General brochure"],
                ["Source", source || "Unknown"],
                ["Message", message || "N/A"],
                ["Received", `${timestamp} (IST)`],
              ].map(([label, value]) => `
              <tr>
                <td style="padding:12px 0;border-bottom:1px solid #f0f0f0;font-size:13px;
                           color:#6b7280;font-weight:600;width:32%;vertical-align:top;">
                  ${label}
                </td>
                <td style="padding:12px 0;border-bottom:1px solid #f0f0f0;font-size:14px;
                           color:#111827;font-weight:500;vertical-align:top;">
                  ${label === "Email"
                    ? `<a href="mailto:${value}" style="color:#059669;text-decoration:none;font-weight:600;">${value}</a>`
                    : value}
                </td>
              </tr>`).join("")}
            </table>
        ...
        <tr>
          <td style="padding:20px 32px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0"
              style="background:#f8faf9;border:1px solid #e2e8f0;border-radius:8px;">
              <tr>
                <td style="padding:16px 18px;">
                  <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#111827;">
                    Quick Reminder
                  </p>
                  <p style="margin:0 0 14px;font-size:12px;color:#6b7280;line-height:1.6;">
                    A potential customer is interested in your brochure. Respond at the earliest to convert this lead.
                  </p>
                  <a href="mailto:${email}?subject=Re%3A%20Your%20Brochure%20Enquiry%20%E2%80%94%20SunTech%20Packaging"
                    style="display:inline-block;background:#f5a623;color:#111827;font-size:12px;
        ...
        <!-- FOOTER -->
        <tr>
          <td align="center" style="background:#fafafa;padding:22px 32px;border-top:1px solid #f0f0f0;">
            <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#111827;">
              SunTech Packaging Machines
            </p>
            <p style="margin:0 0 10px;font-size:12px;color:#6b7280;line-height:1.6;">
              MIDC Industrial Area, Bhiwandi, Maharashtra – 421302<br/>
              <a href="mailto:info@suntech.in" style="color:#e8961a;text-decoration:none;font-weight:600;">info@suntech.in</a>
            </p>
            <p style="margin:0;font-size:10px;color:#9ca3af;letter-spacing:0.05em;text-transform:uppercase;">
    ...
    res.status(200).json({ success: true, message: "Brochure lead sent" });
  } catch (error) {
    const errorMsg = error?.message || String(error);
    console.error("Brochure email sending failed:", errorMsg);
    if (error?.stack) console.error(error.stack);
    fs.appendFileSync("email-error.log", `${new Date().toISOString()} BROCHURE ERROR: ${errorMsg}\n`);
    res.status(500).json({ success: false, error: "Unable to send brochure lead" });
  }
});
